...
const onSubmit = () => {
  try {
    if (data.value.password !== data.value.password_confirmation) {
        notificationStore.showNotification("Password konfirmasi tidak sama!", "error");
        return
    }
    ...
    notificationStore.showNotification("Register Berhasil!", "success");
    router.push("/login");
  } catch (error) {
    console.log(error);
  }
  ...
  <AuthLayout title="Register Akun" back-path="/login">
    <form @submit.prevent="onSubmit">
      ...
      >
        ...
        <template v-if="errors.username" #error>{{
          errors.username[0]
        }}</template>
      </FormInput>
      <FormInput
      ...
      <div class="flex items-center justify-center mb-5">
        <router-link to="/login" class="label-text text-white"
          >Sudah punya akun?
